...
u8 TP_Init(void)
{
  ...
  //没EEPROM直接用默认值校准
		if(!lcddev.dir)//竖屏
		{
			tp_dev.xfac=-0.06585446;//x校准参数			          
			tp_dev.yfac=0.086339809;//y校准参数
				//x偏移量
			tp_dev.xoff=0x0101;			   	  
				//y偏移量
			tp_dev.yoff=0xfff4;				 	  
		...
		{
			tp_dev.xfac=-0.0831107125;//x校准参数			          
			tp_dev.yfac=0.0650829822;//y校准参数
				//x偏移量
			tp_dev.xoff=0x014B;			   	  
				//y偏移量
			tp_dev.yoff=0xFFF8;				 	  
...
//检测指定区域内的屏幕是否被按下
//(x1,y1),(x2,y2):矩形的对角坐标
u8 TP_CheckRectangle(u16 x1, u16 y1, u16 x2, u16 y2)
{	
